...
const Footer: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  const updatedSocialMedia = companyInfo.socialMedia.filter(
    (s) => s.name !== "Twitter" && s.name !== "Instagram"
  )
  ...
  return (
    <footer
      className={`home-container w-full bg-gray-900 text-white font-serif ${className}`}
    >
      <div
      ...
      <div className="border-t border-white/20 py-4">
        <div className="max-w-[1240px] mx-auto flex justify-between text-sm opacity-100">
          ...
          <nav className="flex gap-6">
            ...
            <NavLink to="/services/privacyPolicy" className="hover:opacity-100">
              Privacy Policy
            </NavLink>
          </nav>
        </div>
